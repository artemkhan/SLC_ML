...
option_list = list(
  ...
  make_option("--CLSA", action="store", default=NA, type='character',
              help="Path to dataframe of gwas and eQTL summary statistics [required]"))
...
harmon <- read.xlsx("/ru-auth/local/home/akhan01/MWAS/DeepGeneMAP/Reference files/20230807_HarmonizationMetabolitesProper.xlsx")
harmon <- na.omit(harmon)
df <- df[df$metab_name %in% harmon$`CLSA.cohort.(orignal.names).x`,]

if (grepl("CLSA", dir)) {
  colnames(df)[colnames(df) == "Gene_name"] <- "gene_name" }

metabolite_list <- unique(df$metab_name)
if (end > nrow(df)) { end <- nrow(df)}
rxn_map <- read.xlsx("Reference files/20240319_RxnMap.xlsx")

## See how many neighbors and choose the random number
Sys.time()
